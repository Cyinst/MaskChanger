...
class test_head(BaseDecodeHead):
    def __init__(self,
        ...
        self.query_feat1 = nn.Embedding(self.num_queries, feat_channels)
    ...
    def forward(self, feats, img_metas):
        ...
        for i in range(len(feats1)):
            multi_scale_memorys1.append(feats1[3 - i])
        ...
        mask_pred, attn_mask = self.siamese_forward(
            query_feat1, query_feat2, mask_features, multi_scale_memorys1[0].shape[-2:])
        ...
        for i in range(self.num_transformer_decoder_layers):  # 9
            ...
            # attn_mask[torch.where(
            #     attn_mask.sum(-1) == 0)] = True  # every h*w q
            ...
            query_feat1 = layer(
                query=query_feat1,
                key=decoder_inputs1[level_idx],
                value=decoder_inputs1[level_idx],
            ...
            query_feat2 = layer(
                ...
                key_padding_mask=None)
            mask_pred, attn_mask = self.siamese_forward(
                query_feat1, query_feat2, mask_features, multi_scale_memorys1[
                                                             (i + 1) % self.num_transformer_feat_level].shape[-2:])

            # cls_pred_list.append(cls_pred)
            mask_pred_list.append(mask_pred)
        return mask_pred_list
        # m = mask_features + self.ffn(mask_features)
        # m = self.fusion_conv(m)
        # return m
    ...
    def forward_test(self, inputs, img_metas, test_cfg):
        ...
        """
        seg_logits = self.forward(inputs, img_metas)
        self.save_feature_map(torch.cat(seg_logits, dim=0), 'vis/' + img_metas[0]['filename'][0][-7:-4])
        return seg_logits[-1]

    def save_feature_map(self, feat, save_path='vis', prefix='test'):
        ...
        for i in range(feat.shape[-1]):
            ...
            plt.savefig(os.path.join(save_path, prefix + "_{}.jpg".format(i)))
