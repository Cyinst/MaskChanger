...
class MSDeformAttnPixelDecoder_(BaseModule):
    ...
    def forward(self, feats):
        ...
        for i in range(self.num_encoder_levels):  # 3
            level_idx = self.num_input_levels - i - 1  # (3 -> 1)
            ...
            padding_mask_resized = feat.new_zeros(
                (batch_size,) + feat.shape[-2:], dtype=torch.bool)  # bhw False matrix
            pos_embed = self.postional_encoding(padding_mask_resized)  # (b,num_feats*2(256),h,w)
            level_embed = self.level_encoding.weight[i]  # 256 init
            level_pos_embed = level_embed.view(1, -1, 1, 1) + pos_embed  # (b,num_feats*2(256),h,w)
            # (h_i * w_i, 2)
            reference_points = self.point_generator.single_level_grid_priors(
                feat.shape[-2:], level_idx, device=feat.device)
            # normalize
            factor = feat.new_tensor([[w, h]]) * self.strides[level_idx]  # (1,2)
            ...
            padding_mask_resized = padding_mask_resized.flatten(1)  # b,h_i*w_i
        ...
        encoder_inputs = torch.cat(encoder_input_list, dim=0)  # low to high hw embeddings concat
        ...
        level_start_index = torch.cat((spatial_shapes.new_zeros(
            (1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))  # (0, lowest h*w)
        ...
        for i in range(self.num_input_levels - self.num_encoder_levels - 1, -1,
                       -1):  # 0
            x = feats[i]
            cur_feat = self.lateral_convs[i](x)  # (b,c 256,h,w)
